...
This document records the stable top-level JSON shapes for the current CLI surface. Nested record fields follow the public TypeScript model in `src/types.ts` unless noted otherwise. Stability labels, compatibility rules, and 1.0 readiness criteria are tracked in [Public_Contracts.md](Public_Contracts.md).
