...
| **团队工作流 Artifact** | 创建私有图 bundle、校验 bundle freshness，并生成适合 CI 使用的报告；不扩展 MCP 工具面。 |
...
node dist/bin/mdgraph.js semantic status --json         # 语义 provider、向量覆盖率和重建指引
...
node dist/bin/mdgraph.js eval --query-set cjk --path /path/to/project --json # CJK 检索基线
node dist/bin/mdgraph.js eval --query-mode semantic --json # 评估可选语义 reranking 诊断

# 团队工作流 artifact
node dist/bin/mdgraph.js bundle create --profile private --json # 创建私有目录 bundle
node dist/bin/mdgraph.js bundle verify .mdgraph/bundles/private/BUNDLE_DIR --json # 校验 bundle 完整性和 freshness
node dist/bin/mdgraph.js report --json                          # counts、storage、schema、doctor summary 和 hashes
node dist/bin/mdgraph.js report --json --eval --bundle .mdgraph/bundles/private/BUNDLE_DIR # 包含 eval 和 bundle verification
node dist/bin/mdgraph.js diff --base main --json                # PR 文档图影响摘要
node dist/bin/mdgraph.js report --json --base main              # 在 report 中包含 diff summary
node dist/bin/mdgraph.js report --json --benchmark benchmark-runs.json # 包含 paired agent run-record delta

# 互操作导出
node dist/bin/mdgraph.js export graphjson --json                # 确定性的结构化 GraphJSON
node dist/bin/mdgraph.js import graphjson graph.json --verify --json # 只校验 GraphJSON，不写入索引
node dist/bin/mdgraph.js export mermaid trace "AuthService" "RedisTimeoutError" # Mermaid trace 图
node dist/bin/mdgraph.js export markdown-index                  # Obsidian-friendly Markdown 图索引
node dist/bin/mdgraph.js export docs-site --json                # 轻量 docs-site 数据
node dist/bin/mdgraph.js export source-bridge --provider codegraph --artifact codegraph.json --json # 只读 source bridge 报告
...
`diff --base <ref>` 会为 base Git revision 构建隔离临时索引，并与当前 graph index 比较。它报告新增、修改、删除、重命名的 Markdown 文档、图计数 delta、doctor warning delta、变更 source refs 和简短 PR summary。它不会检查源码 AST，也不会替换当前 `.mdgraph/graph.db`。

`report --benchmark <file>` 读取结构化 with/without-MDGraph agent run records，并嵌入 file reads、text searches、tool calls、MDGraph calls、字符/token 预算、延迟、raw file fallback 和引用正确率的 paired delta。它不解析完整 transcript，也不运行 agent。不完整 pair 会进入 skipped；`unknown` 引用不进入正确率分母。

`export graphjson` 输出确定性的结构化图导出，供其他工具读取。它包含 documents、sections、entities、source refs 和 edges，带稳定排序和 `graphHash`；刻意不包含 chunk 正文、vectors、SQLite 内部细节或绝对项目根目录。`import graphjson --verify` 只校验导出形状和 hash，不会把外部 graph 合并进 `.mdgraph/graph.db`。

`export mermaid trace` 把现有 graph trace 渲染成确定性的 Mermaid。`export markdown-index` 和 `export docs-site --json` 为 Obsidian 风格笔记和 docs site 系统生成 adapter 数据，但不运行站点生成器。`export source-bridge --provider codegraph` 读取显式传入的本地 CodeGraph-style JSON artifact 并报告 source-ref 匹配；artifact 缺失时返回 unsupported，bridge 输出不会影响 indexing、search、context 或 MCP 工具。

### 团队工作流示例

CI changed-document gate 可运行一次新索引，并把 doctor 限定到 PR 或 push base ref：
...
仓库 CI workflow 已包含可复制的 GitHub Actions 示例：安装、构建、测试和 CLI smoke 后，运行 `index`、`doctor --since <base> --fail-on warn`、`eval`、`bundle create`、`bundle verify` 和 `report --eval --bundle`，再上传 `.mdgraph/reports/**` 与 `.mdgraph/bundles/private/**` artifact。

可复用的私有 CI artifact 流程：先创建 bundle，把目录作为 build artifact 保存，再在后续 job 中把该目录传给 `bundle verify` 或 `report --bundle`。

示例 `.git/hooks/pre-commit` changed-doc gate：
...
示例 `.git/hooks/pre-push` error-only gate：
...
两个 hook 都会先重建本地索引再检查，避免依赖 stale `.mdgraph/graph.db`。pre-commit 通过 `--changed` 检查 staged/worktree Markdown；pre-push 通过 `--since` 检查已提交的分支变更。紧急本地绕过可设置 `MDGRAPH_SKIP_HOOK=1`，合并前仍应运行 CI workflow。

PR 文档影响摘要可先在当前 checkout 上运行 `node dist/bin/mdgraph.js index --json`，再运行 `node dist/bin/mdgraph.js diff --base origin/main --json` 或 `report --json --base origin/main`。

A/B benchmark report 可为同一个 `questionId` 捕获一个 `with_mdgraph` 和一个 `without_mdgraph` 结构化 run record，然后运行 `node dist/bin/mdgraph.js report --json --benchmark benchmark-runs.json`。完整 transcript 应保留在公开 artifact 之外。
...
MCP 服务器会绑定到 `serve --path` 项目根。客户端传入的 `rootUri`、`workspaceFolders` 和工具 `projectPath` 都必须位于该服务根之内；手动 MCP 预算会限制 search limit、trace depth 和 context 字符数。
...
会放大资源消耗的数字字段在加载时有硬上限：`index.maxFileBytes` <= 10 MiB，`search.defaultLimit` <= 100，`search.maxDepth` <= 12，`search.maxContextChars` <= 200000，`search.highFrequencyEntityThreshold` <= 100000，`embedding.dimensions` <= 4096。
...
| 表 | 记录 |
|----|------|
| `schema_metadata` | Schema version、MDGraph 版本来源、更新时间以及 `current`/`legacy` baseline metadata |
| `schema_migrations` | 为未来 schema migration 预留的审计表 |
...
| `chunk_vectors` | 以紧凑 Float32 BLOB 行存储的可选本地语义向量（默认 128 维） |
...
| 模块 | 路径 | 职责 |
|------|------|------|
| CLI | `src/bin/mdgraph.ts` | 基于 Commander 的 CLI — init、index、status、search、context、node、trace、eval、diff、bundle、report、export、import、serve、watch、doctor |
...
| Benchmark | `src/benchmark/*` | 结构化 agent A/B run-record 解析和 paired delta 聚合 |
| Bundle | `src/bundle/*` | 私有目录图 bundle 创建和校验 |
| Export/Import | `src/export/*` | 确定性 GraphJSON、Mermaid、Markdown/docs-site 和只读 bridge adapter |
| Reporting | `src/reporting/*` | 面向 CI 的图工作流报告聚合 |
...
- **互操作优先是 adapter** — GraphJSON、Mermaid、Markdown/docs-site export 和 source bridge report 都是只读导向的 adapter，不会把核心索引器扩展成图数据库平台或源码分析系统。
