...
| 工具 | 用途 | 使用时机 |
|------|------|----------|
| `mdgraph_search` | 按关键字或实体名称搜索文档、章节和实体 | 快速查找 — 在读取文件之前 |
| `mdgraph_context` | 为跨文档问题构建可解释的上下文包；MCP 支持 `knownFiles` 和 `maxChars` 用于 task-start brief | 理解某个功能、调试流程 — 需要来自多个文件的文档 |
...
| `mdgraph_status` | 报告索引可用性、计数、数据库路径和轻量 last-indexed freshness metadata | 在依赖索引之前验证它是否活跃 |
...
- **快速关键字或实体查找使用 `mdgraph_search`** — 结果按相关性排序，匹配的实体高亮显示。
- **当你知道要查找什么时使用 `mdgraph_node`** — 按名称、路径、`docs/file.md#anchor` 或图谱 ID 解析。
- **关系类问题使用 `mdgraph_trace`** — 返回路径的每一步，包含边类型、来源和置信度。
- **把 `mdgraph_status` 作为轻量 readiness check** — 它不会扫描文件确认 stale changes；Markdown 变更后请运行 `mdgraph doctor --json` 或 `mdgraph index`。
