...
所有查询命令都支持 `--json`，为代理和脚本提供结构化输出。`mdgraph eval` 报告 search/context/trace 质量的轻量检索指标；它是确定性 smoke 检查，不是真实 agent A/B benchmark。稳定的顶层字段记录在 [Output_Contracts.md](docs/ZH/Output_Contracts.md)。
...
| `mdgraph_context` | 为跨文档问题构建可解释的上下文包 | 理解某个功能、调试流程 — 需要来自多个文件的文档 |
...
| `mdgraph_status` | 报告索引健康度、计数和数据库路径 | 在依赖索引之前验证它是否活跃 |
