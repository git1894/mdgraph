...
3. If status is active, use its counts and freshness metadata as a lightweight readiness check.
4. If the user asks about stale files, broken docs, CI gates, or health risks, run CLI `mdgraph doctor --json` rather than relying on `mdgraph_status` alone.
